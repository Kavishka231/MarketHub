...
export default function ProductDetailPage() {
  ...
  return <main className="mx-auto max-w-6xl px-6 py-10"><nav className="text-sm text-slate-500"><Link className="hover:text-brand-700" to="/products">Products</Link><span> / {product.name}</span></nav><div className="mt-8 grid gap-10 md:grid-cols-2"><ProductGallery imageUrl={product.imageUrl} name={product.name}/><div><ProductInformation product={product}/><div className="mt-4"><WishlistButton productId={product.id}/></div><PurchasePanel product={product}/></div></div></main>;
}
