  ...
  if (query.isError) {
    ...
    return error.status === 404 ? (
      ...
      <main className="mx-auto max-w-4xl px-6 py-16">
        <ErrorState message={error.message} onRetry={() => query.refetch()} />
      </main>
    );
  }
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-10">
      <nav className="text-sm text-slate-500">
        <Link className="hover:text-brand-700" to="/products">
          Products
        </Link>
...
function NotFound() {
  return (
    <main className="mx-auto max-w-xl px-6 py-20 text-center">
      <p className="text-sm font-semibold text-brand-600">Product unavailable</p>
      <h1 className="mt-2 text-3xl font-bold">This product could not be found</h1>
      <p className="mt-3 text-slate-600">
        It may have been removed or is no longer publicly available.
      </p>
      <Link className="mt-6 inline-block text-brand-700 underline" to="/products">
        Back to products
      </Link>
    </main>
  );
