...
it("searching updates product query state", async () => { show(); await screen.findByText("Ceylon Tea"); fireEvent.change(screen.getByLabelText("Search products"), { target: { value: "tea" } }); await waitFor(() => expect(api.fetchProducts).toHaveBeenLastCalledWith(expect.objectContaining({ search: "tea", page: 0 }))); });
