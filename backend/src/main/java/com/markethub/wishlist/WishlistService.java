...
import com.markethub.product.*; import com.markethub.user.*; import org.springframework.data.domain.*; import org.springframework.stereotype.Service; import org.springframework.transaction.annotation.Transactional;
@Service
public class WishlistService {
 ...
 @Transactional(readOnly=true) public WishlistPageResponse list(String email,int page,int size){User c=customer(email);Page<WishlistItem> result=items.findByCustomerId(c.getId(),PageRequest.of(page,size,Sort.by("createdAt").descending()));return WishlistPageResponse.from(result);}
